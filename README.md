...
| **Gallery albums** | Photo albums. Link an album to its event and the two pages point to each other. Newer albums fill the film strip on the home page. |
| **ProduceUMI songs** | Songs made in the ProduceUMI workshop. Set the **School year** (2025 means 2025/26) and the ProduceUMI page groups them into "This year" and "Last year". The three newest appear on the home page. Paste the song's **Spotify link** to add a Spotify button everywhere the song is listed (and a Spotify player on its page). Upload a 15–30 second MP3 as the **Song snippet** and clicking the record plays it; one snippet plays at a time. **Listen link** is optional, for a SoundCloud or YouTube player instead. |
...
| **Pages** | Headings, paragraphs, photos, prices, FAQs and steps on Home, ProduceUMI, About, Join, Booking & Rentals, Lessons, Sponsors and Contact. |
...
7. **Connect the domain.** Netlify → *Domain management → Add a domain* → `ubcmusicinitiative.com`, then update the DNS records where the domain is registered (currently Squarespace). HTTPS is set up automatically. Old Squarespace links such as `/joinus` and `/pastevents` already redirect to the new pages.
8. **Daily rebuild (recommended).** This keeps "upcoming" accurate on quiet weeks.
   1. In Netlify, go to *Build & deploy → Build hooks* and create a hook.
   2. In GitHub, go to *Settings → Secrets → Actions* and add a secret named `NETLIFY_BUILD_HOOK` containing the hook URL.
   3. `.github/workflows/daily-rebuild.yml` does the rest.
...
- **Upcoming vs. past:** decided at build time, then corrected in the browser, so a show that ended since the last deploy won't appear as upcoming. The upcoming lists cover the next 21 days by Vancouver date (`UPCOMING_DAYS` in `src/lib/upcoming.ts`); later events are rendered hidden so the browser can roll the list forward between deploys. The daily rebuild keeps the HTML itself fresh.
